...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.swag_diag_means = self._create_weight_copy()
        self.swag_diagonal_means_squared = self._create_weight_copy()
        self.swag_diagonal_count = 0
    ...
    def update_swag_statistics(self) -> None:
        ...
        for name, param in copied_params.items():
            ...
            self.swag_diag_means[name] = (self.swag_diag_means[name] * self.swag_diagonal_count + 
                                          param) / (self.swag_diagonal_count + 1)
            self.swag_diagonal_means_squared[name] = (self.swag_diagonal_means_squared[name] * self.swag_diagonal_count + 
                                          param**2) / (self.swag_diagonal_count + 1)
        ...
        # Full SWAG
            ...
            # TODO(2): update full SWAG attributes for weight `name` using `copied_params` and `param`
              #  for name, param in copied_params.items():
                    self.full_diag[name].append(param -  self.swag_diag_means[name])
            #raise NotImplementedError("Update full SWAG statistics")
        self.swag_diagonal_count += 1
    ...
    def fit_swag_model(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        #self.initialize_weights(self.network)
        self.network.train()
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.num_bma_samples, desc="Performing Bayesian model averaging"):
            ...
            with torch.no_grad():
                for batch_images, *_ in loader:
                    # ensure that the batch_images tensor is on the same device (CPU or GPU) as the model
                    # self.network, before performing any operations.
                    batch_images = batch_images.to(next(self.network.parameters()).device)
                    logits = self.network(batch_images)
                    # computing prob for each row
        ...
        # Stack the tensors into a single tensor of shape (num_bma_samples, N, 6)
        stacked_tensors = torch.stack(model_predictions)
        bma_probabilities = torch.mean(stacked_tensors, dim=0)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            # SWAG-diagonal part
            z_diag = torch.randn(param.size()) #this already generates a random vector from N(0, I)
            # TODO(1): Sample parameter values for SWAG-diagonal
            #raise NotImplementedError("Sample parameter for SWAG-diagonal")
            mean_weights = self.swag_diag_means[name]
            std_weights = (1 / math.sqrt(2)) * torch.sqrt(self.swag_diagonal_means_squared[name] - self.swag_diag_means[name] ** 2)
            ...
            if self.inference_mode == InferenceType.SWAG_FULL:
                ...
                for i in range(self.max_rank_deviation_matrix):
                    sampled_weight += 1/math.sqrt(2 * (self.max_rank_deviation_matrix - 1)) * self.full_diag[name][i] * z_full[i]  
                # assert D.ndim == 2 and D.size(0) == param.size() and D.size(1) == self.max_rank_deviation_matrix 
...
def evaluate(
    ...
    # 1 element tensor to single scalar
