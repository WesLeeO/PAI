...
class Model(object):
    ...
    def findCluster(self, v):
        ...
        for i in range(self.num_clusters):
            distance = np.linalg.norm(v - self.models[i][1])
    ...
    def generate_predictions(self, test_coordinates: np.ndarray, test_area_flags: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = np.empty((test_area_flags.shape))
        means = np.empty((test_area_flags.shape))
        stds =  np.empty((test_area_flags.shape))

        for i in range(test_coordinates.shape[0]):
            v = test_coordinates[i]
            index = self.findCluster(v)
            gp_mean, gp_std = self.models[index][0].predict(v.reshape(1, 2), return_std=True)
            if test_area_flags[i] == 0:
                predictions[i] = gp_mean[0]
            else:
                predictions[i] = gp_mean[0] + norm.ppf(COST_W_UNDERPREDICT / (COST_W_UNDERPREDICT + COST_W_NORMAL)) * gp_std[0]
            means[i] = gp_mean[0]
            stds[i] = gp_std[0]

        return predictions, means, stds
    ...
    def train_model(self, train_targets: np.ndarray, train_coordinates: np.ndarray, train_area_flags: np.ndarray):
        ...
        kernel = ConstantKernel() * RBF(length_scale_bounds=(1e-3, 1e1)) + WhiteKernel(noise_level_bounds=(1e-2, 1e1))
        ...
        for i in range(self.num_clusters):
            samples = train_coordinates[labels == i]
            targets = train_targets[labels == i]
            flags =  train_area_flags[labels == i]
            n_samples = int(0.5 * samples.shape[0])
            indices_res = np.where(flags == 1)[0]
            indices_non_res = np.where(flags == 0)[0]
            subset_indices_res = np.random.choice(indices_res, int(0.5 * n_samples), replace=False)
            subset_indices_non_res = np.random.choice(indices_non_res, int(0.5 * n_samples), replace=False)
            indices = np.concatenate((subset_indices_res, subset_indices_non_res))
            subset_samples = samples[indices]
            subset_targets = targets[indices]
            gpr = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=2).fit(subset_samples, subset_targets)
            print(gpr.kernel_)
            models.append((gpr, kmeans.cluster_centers_[i]))
...
def extract_area_information(train_x: np.ndarray, test_x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    train_area_flags = train_x[:, -1]
    test_coordinates = test_x[:, :2]
    test_area_flags = test_x[:, -1]
